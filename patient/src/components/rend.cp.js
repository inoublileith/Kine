...
const Rend = () => {
  ...
  console.log(id)
  const retrieveRends = () => {
    RendDataService.getAllRendp(id)
      .then((response) => {
  ...
  const change =(id,etat) => {
    if (etat==0){
     return <button onClick={() => supprimer(id)}>Supprimer</button>
    }
    else if (etat==1){
     return (
       <>
         <label>rendez-vous accepter</label>
         <button onClick={() => supprimer(id)}>Annuler</button>
       </>
     )
    }
 ...
 KineDataService.get(id)
  ...
  const RendCard = ({idrend, date,etat ,heure,nom,prenom,tel}) => {
  return (
      ...
      <div className='dc-article'>
        ...
        <div className='dc-articlecontent'>
          <div className='dc-title dc-ellipsis dc-titlep'>
            ...
            <h3>
              <a href='javascript:void(0);'>{change(idrend, etat)}</a>
            </h3>
          </div>
        </div>
      </div>
