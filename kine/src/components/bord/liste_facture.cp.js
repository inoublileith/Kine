...
const ShowFactureBords = (props) => {
  ...
  const retrieveFactures = (id) => {
    FactureDataService.getbybord(id)
      .then((response) => {
...
//  let somme = 0 
//   {
//     factures.map((facture, i) => (somme = somme + facture.prix_ht))
//    }
//    return(somme)
// }
// console.log(calcule())
...
//   }
  ...
  const deleteFacture = (rowIndex, x) => {
    const id = facturesRef.current[rowIndex].id
    FactureDataService.delete(id)
      .then(() => {
        refreshList(id)
...
export default ShowFactureBords
